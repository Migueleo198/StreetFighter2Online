import { FighterDirection, FighterState,FighterAttackType, FighterAttackStrength, FighterAttackBaseData } from "../application/constantsFighter.js";
...
class Sprite {
    constructor(x, y, imageSrc, direction, playerId, onAttackHit) {
    ...
    updateAttackBoxCollided(time) {
        ...
        for (const hurt of this.opponent.boxes.hurt) {
            ...
            if (boxOverlap(actualHitBox, actualOpponentHurtBox)) {
                ...
                const hitPosition = {
                    x: (actualHitBox.x + (actualHitBox.width /2 ) + actualOpponentHurtBox.x + (actualOpponentHurtBox.width /2))/2+offsetX*this.direction,
                    y:(actualHitBox.y + (actualHitBox.height /2 ) + actualOpponentHurtBox.y + (actualOpponentHurtBox.height /2))/2 +offsetY,
                }
